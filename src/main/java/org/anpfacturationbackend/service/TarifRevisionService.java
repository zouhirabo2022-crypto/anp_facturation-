...
import org.anpfacturationbackend.entity.TarifConcession;
...
public class TarifRevisionService {
    ...
    /**
     * Ex̩cute la r̩vision annuelle des tarifs.
     * Cette m̩thode doit ̻tre appel̩e par un scheduler (cron) ou manuellement par
     * un admin.
     * 
     * @param targetYear L'ann̩e pour laquelle g̩n̩rer les nouveaux tarifs
     *                   (g̩n̩ralement l'ann̩e courante).
     * @return Le nombre de tarifs r̩vis̩s.
     */
        ...
        for (TarifOTDP oldTarif : activeTarifs) {
            if (shouldRevise(oldTarif.getAnneeTarif(), oldTarif.getAnneeDebutRevision(), oldTarif.getDelaiRevision(),
                ...
                auditService.log("REVISION_TARIF",
                        "R̩vision OTDP: " + oldTarif.getId() + " -> " + newTarif.getId() + " (" + nouveauMontant + ")");
                count++;
            }
        }
        return count;
    ...
    public int reviseTarifsEauElectricite(int targetYear) {
        ...
        for (TarifEauElectricite oldTarif : activeTarifs) {
            if (shouldRevise(oldTarif.getAnneeTarif(), oldTarif.getAnneeDebutRevision(), oldTarif.getDelaiRevision(),
                ...
                auditService.log("REVISION_TARIF", "R̩vision Eau/Elec: " + oldTarif.getId() + " -> " + newTarif.getId()
                        + " (" + nouveauTarifFacture + ")");
                count++;
            }
        }
    ...
    public int reviseTarifsAutorisation(int targetYear) {
        ...
        for (TarifAutorisation oldTarif : activeTarifs) {
            if (shouldRevise(oldTarif.getAnneeTarif(), oldTarif.getAnneeDebutRevision(), oldTarif.getDelaiRevision(),
                ...
                auditService.log("REVISION_TARIF", "R̩vision Autorisation: " + oldTarif.getId() + " -> "
    ...
    public int reviseTarifsConcession(int targetYear) {
        ...
        for (TarifConcession oldTarif : activeTarifs) {
            if (shouldRevise(oldTarif.getAnneeTarif(), oldTarif.getAnneeDebutRevision(), oldTarif.getDelaiRevision(),
                ...
                auditService.log("REVISION_TARIF", "R̩vision Concession: " + oldTarif.getId() + " -> "
                        + newTarif.getId() + " (" + nouveauMontant + ")");
                count++;
            }
        }
